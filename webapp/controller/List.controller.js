sap.ui.define([
	...
	return BaseController.extend("transporte.apptransporte.controller.List", {
		...
		onInit : function () {
			...
			this._oGroupFunctions = {
				value: function(oContext) {
					var iNumber = oContext.getProperty('value'),
						key, text;
					if (iNumber <= 20) {
						key = "LE20";
		...
		onSearch: function (oEvent) {
			...
			if (sQuery) {
				this._oListFilterState.aSearch = [new Filter("transportid", FilterOperator.Contains, sQuery)];
			} else {
		...
		onOpenViewSettings: function (oEvent) {
			...
			if (!this.byId("viewSettingsDialog")) {
				Fragment.load({
					...
					name: "transporte.apptransporte.view.ViewSettingsDialog",
					controller: this
		...
		_showDetail: function (oItem) {
			...
			this.getRouter().navTo("object", {
				objectId : oItem.getBindingContext().getProperty("transportid")
			}, bReplace);
		},
